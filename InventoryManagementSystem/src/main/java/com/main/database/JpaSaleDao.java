...
public class JpaSaleDao extends JpaDataAccessObject<SaleEntity> {
    ...
    public Optional<SaleEntity> get(int id) {
        return Optional.ofNullable(EntityManagerConnector.getEntityManager().find(SaleEntity.class, id));
    }

    @Override
    public List<SaleEntity> getAll() {
        return EntityManagerConnector.getEntityManager().createQuery("SELECT e FROM SaleEntity e", SaleEntity.class).getResultList();
    }

    public List<SaleEntity> getAllByDate(java.sql.Date date) {
        String query = "SELECT e FROM SaleEntity e WHERE e.date = :date";
        return EntityManagerConnector.getEntityManager().createQuery(query, SaleEntity.class).setParameter("date", date).getResultList();
    }

    public List<SaleEntity> getAllByPrice(double price) {
        String query = "SELECT e FROM SaleEntity e WHERE e.price = :price";
        return EntityManagerConnector.getEntityManager().createQuery(query, SaleEntity.class).setParameter("price", price).getResultList();
    }
    ...
    public List<SaleEntity> getAllByProductName(String productName) {
        ...
        return EntityManagerConnector.getEntityManager().createQuery(query, SaleEntity.class).setParameter("productName", productName).getResultList();
    }
